...
def scan_and_publish(config: dict):
    ...
    try:
        for ts_key in get_raw_keys(config):
            mp4_key = ts_key.rsplit('.', 1)[0] + '.mp4'
            if is_processed(mp4_key, config): continue
            if is_in_dlq(ts_key, config): continue
            if _already_converted(ts_key, config): continue
            if _is_in_flight(ts_key, ttl): continue
        ...
        for mp4_key in get_input_mp4_keys(config):
            if is_processed(mp4_key, config): continue
            if is_in_dlq(mp4_key, config): continue
            if _is_in_flight(mp4_key, ttl): continue
...
if __name__ == '__main__':
    ...
    status_interval = watcher_cfg.get('status_interval', 0)
    last_status     = 0.0
