...
def _scan_bucket(s3, bucket: str, extension: str, min_age_seconds: int = 0) -> list:
    targets = []
    now = datetime.now(timezone.utc)
    paginator = s3.get_paginator('list_objects_v2')
    for page in paginator.paginate(Bucket=bucket):
        for obj in page.get('Contents', []):
            if not obj['Key'].upper().endswith(extension.upper()):
                continue
...
def get_converted_keys(config: dict, min_age_seconds: int = 0) -> list:
    """Return .MP4 files from the converted bucket (ready for analysis).

    min_age_seconds: skip files younger than this (0 = return all).
    Phase 2 recovery passes a value here so the watcher doesn't re-queue
    files that a worker is actively processing right now.
    """
    s3 = _get_s3_client(config)
    bucket = config['storage']['buckets']['converted']
    try:
        keys = _scan_bucket(s3, bucket, '.mp4', min_age_seconds=min_age_seconds)
